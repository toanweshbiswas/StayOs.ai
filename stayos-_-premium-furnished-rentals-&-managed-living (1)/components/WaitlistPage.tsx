...
const WaitlistPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.firstName || !formData.lastName || !formData.email || !role) {
            ...
            <div className="w-full max-w-3xl relative z-10 glass-panel shadow-2xl p-8 md:p-12 rounded-xl border border-ink/10 mb-24">
                ...
                {role && (
                    <div className="animate-fade-in-up">
                        {status === 'success' ? (
                            <div className="text-center py-12 border border-black/10 rounded-lg bg-green-50/50">
                                <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <Check size={32} />
                                </div>
                                <h3 className="text-2xl font-serif text-black mb-2">Application Received</h3>
                                <p className="text-black/60 font-serif">We'll review your application and be in touch soon.</p>
                            </div>
                        ) : (
                            <>
                                <form className="space-y-0 border border-black/10 rounded-lg overflow-hidden" onSubmit={handleSubmit}>
                                    <div className="grid grid-cols-1 md:grid-cols-2">
                                        <div className="border-b md:border-b-0 md:border-r border-black/10 p-4">
                                            ...
                                            <input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} className="w-full bg-transparent border-b border-black/20 text-black py-2 focus:outline-none focus:border-black transition-colors" placeholder="Jane" required />
                                        </div>
                                        <div className="p-4 border-b border-black/10 md:border-b-0">
                                            ...
                                            <input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} className="w-full bg-transparent border-b border-black/20 text-black py-2 focus:outline-none focus:border-black transition-colors" placeholder="Doe" required />
                                        </div>
                                    </div>

                                    <div className="p-4 border-t border-black/10">
                                        <label className="block text-xs font-bold text-black/40 uppercase tracking-widest mb-2">Email Address *</label>
                                        <input type="email" name="email" value={formData.email} onChange={handleInputChange} className="w-full bg-transparent border-b border-black/20 text-black py-2 focus:outline-none focus:border-black transition-colors" placeholder="jane@example.com" required />
                                    </div>

                                    {role === 'tenant' ? (
                                        <div className="p-4 border-t border-black/10">
                                            <label className="block text-xs font-bold text-black/40 uppercase tracking-widest mb-2">Preferred Location</label>
                                            <select name="preferredLocation" value={formData.preferredLocation} onChange={handleInputChange} className="w-full bg-transparent border-b border-black/20 text-black py-2 focus:outline-none focus:border-black transition-colors appearance-none rounded-none">
                                                <option value="Indiranagar" className="bg-white">Indiranagar</option>
                                                <option value="Koramangala" className="bg-white">Koramangala</option>
                                                <option value="HSR Layout" className="bg-white">HSR Layout</option>
                                            </select>
                                        </div>
                                    ) : (
                                        <div className="p-4 border-t border-black/10">
                                            <label className="block text-xs font-bold text-black/40 uppercase tracking-widest mb-2">Property Link</label>
                                            <input type="url" name="propertyLink" value={formData.propertyLink} onChange={handleInputChange} className="w-full bg-transparent border-b border-black/20 text-black py-2 focus:outline-none focus:border-black transition-colors" placeholder="URL" />
                                        </div>
                                    )}

                                    {status === 'error' && (
                                        <div className="p-4 bg-red-50 text-red-600 text-sm font-bold border-t border-black/10">
                                            {errorMessage}
                                        </div>
                                    )}
                                </form>
                                ...
                                <div className="mt-8 flex justify-end">
                                    <button
                                        ...
                                        className="bg-clay text-white px-10 py-4 text-sm font-bold uppercase tracking-widest hover:bg-teal transition-colors flex items-center gap-2 rounded-full shadow-lg disabled:opacity-70"
                                    >
                                        {status === 'loading' ? <><Loader2 size={16} className="animate-spin" /> Submitting...</> : <>Submit Application <ArrowRight size={16} /></>}
                                    </button>
                                </div>
                            </>
                        )}
