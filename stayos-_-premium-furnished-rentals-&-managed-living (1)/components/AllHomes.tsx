...
import { MapPin, BedDouble, Copy, ArrowUpRight, ChevronDown, Check, Zap, Loader2 } from 'lucide-react';
...
const AllHomes: React.FC = () => {
    ...
    const locations = useMemo(() => {
        const locs = new Set(homes.map(h => h.location));
    ...
    const filteredHomes = useMemo(() => {
        return homes.filter(home => {
            ...
            const price = typeof home.price === 'number' ? home.price : parseInt(home.price?.replace(/,/g, '') || '0');
    ...
    }, [homes, budget, location, showAvailable, moveInReady]);
    ...
    return (
        <div className="pt-32 pb-20 px-4 md:px-8 max-w-[1440px] mx-auto min-h-screen">
            ...
            <div className="flex justify-between items-end mb-8 px-2 pb-2 border-b border-ink/10">
                <h2 className="text-ink text-xl font-serif italic">Displaying {isLoading ? '...' : filteredHomes.length} Residences</h2>
            </div>

            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-32 text-center border border-ink/10 border-dashed rounded-lg">
                    <Loader2 size={48} className="animate-spin text-clay mb-4" />
                    <h3 className="text-2xl font-serif text-ink">Loading Core Collection</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border-t border-l border-ink/10">
                    {filteredHomes.map(home => (
                        <Link
                            key={home.id}
                            to={`/listings/${home.id}`}
                            className="group border-r border-b border-ink/10 bg-paper hover:bg-[#F5F5F0] transition-colors p-6 flex flex-col cursor-pointer"
                        >
                            <div className="relative aspect-[4/3] overflow-hidden mb-6 border border-ink/10 p-1 bg-white shadow-sm">
                                <div className="w-full h-full relative overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-700">
                                    <ImageWithLoader
                                        src={home.images && home.images.length > 0 ? home.images[0].url : 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&q=80&w=2675'}
                                        alt={home.name}
                                        className="w-full h-full object-cover"
                                    />
                                </div>
                                {home.images?.length > 1 && (
                                    <div className="absolute bottom-3 right-3 bg-black/50 text-white text-[9px] font-bold uppercase tracking-widest px-2 py-0.5">
                                        +{home.images.length - 1} rooms
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex-1 flex flex-col justify-between">
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 w-full mb-4">
                                        <div className={`w-1.5 h-1.5 rounded-full ${home.availableRooms?.includes('Fully Booked') ? 'bg-red-500' : 'bg-teal'}`} />
                                ...
                                <div className="flex justify-between items-end pt-4">
                                    <div>
                                        ...
                                        <h4 className="text-xl font-serif text-ink">₹{home.price?.toLocaleString()}</h4>
                                    </div>
                                    <span className="border border-ink/20 group-hover:bg-clay group-hover:text-white group-hover:border-clay text-ink px-4 py-2 text-sm font-bold uppercase tracking-widest transition-colors flex items-center gap-2 rounded-sm">
