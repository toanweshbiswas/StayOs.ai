...
const HomeCarousel = forwardRef<HomeCarouselHandle, HomeCarouselProps>(({ 
  ...
  useImperativeHandle(ref, () => ({
    setProgress: (globalProgress: number) => {
      ...
      homes.forEach((_, index) => {
          ...
          const denom = myEnd - myStart;
          let localProgress = denom > 0 ? (scrollProgress - myStart) / denom : 0;
      ...
      if (ctaRef.current) {
          ...
          let localProgress = denom > 0 ? (scrollProgress - start) / denom : 0;
          localProgress = Math.max(0, Math.min(1, localProgress));
          const ease = easeOutQuart(localProgress);
          const startX = 80; 
          const currentX = startX * (1 - ease);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const denom = end - start;
      let progress = denom > 0 ? (scrollY - start) / denom : 0;
      ...
      rafId = requestAnimationFrame(() => {
          ...
          homes.forEach((_, index) => {
              ...
              const myEnd = myStart + 0.3; 
              const denom = myEnd - myStart;
              let p = denom > 0 ? (progress - myStart) / denom : 0;
          ...
          if (ctaRef.current) {
               ...
               const denom = end - start;
               let p = denom > 0 ? (progress - start) / denom : 0;
               p = Math.max(0, Math.min(1, p));
               const ease = easeOutQuart(p);
